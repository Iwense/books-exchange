...
const texts = {
    ...
    thirdName: 'Отчество обязательное поле',
...
const nickNameMatches = /^([a-zA-Z][a-zA-Z0-9-_\.]+)|([А-Яа-я][А-Яа-я0-9-_\.]+){1,50}$/
const passwordMatches = /^((?=.*?[A-Z])(?=.*?[a-z]))|((?=.*?[А-Я])(?=.*?[а-я]))(?=.*?[0-9])[a-zA-ZА-Яа-я\d]$/
...
export const VALIDATION = {
    ...
    SIGN_UP: yup.object().shape({
        ...
        .matches(emailMatches, "Email не корректен ")
        .required("Email обязательное поле"),
        password: yup.string().trim().required(texts.pass).min(8, 'Минимальная длина 8 символов').max(64).matches(passwordMatches, 'Пароль должен содержать заглавную букву и число'),
        ...
        thirdName: yup.string().trim().max(25, 'Максимальная длина 25 символов').required(texts.thirdName).matches(/^[-а-яА-ЯёЁ]+$/, 'Разрешена только кириллица'),
    ...
    PERSONAL_DATA: yup.object().shape({
        ...
        city: yup.string().trim().max(20, 'Максимальная длина 20 символов').required(texts.city).matches(/^[-а-яА-ЯёЁ]+$/, 'Разрешена только кириллица'),
        nickname: yup.string().required(texts.nickname).matches(nickNameMatches, 'Вы используете запрещенные символы').trim().max(50, 'Максимальная длина 50 символов'),
        street: yup.string().trim().max(25, 'Максимальная длина 25 символов').required(texts.street).matches(/^[а-яА-Я\d- ]+$/, 'Разрешена только кириллица и цифры'),
        homeNumber: yup.string().trim().max(5, 'Максимальная длина 5 символов').required(texts.home).matches(/\d+[а-я]{0,1}/i, 'Разрешены только цифры и одна буква'),
        buildNumber: yup.string().trim().max(3, 'Максимальная длина 3 символа').matches(/[\d]{0,2}[а-я]{0,1}$/, 'Разрешены только цифры и одна буква'),
        flatNumber: yup.string().trim().max(3, 'Максимальная длина 3 символа').matches(/[\d ]{0,3}$/, 'Разрешены только цифры'),
    ...
    BOOK_INFO: yup.object().shape({
        ...
        book: yup.string().required(texts.book).matches(/^([a-zA-Z0-9][a-zA-Z0-9-_\.]+)|([А-Яа-я0-9][А-Яа-я0-9-_\.]+){1,50}$/, 'Вы используете запрещенные символы').trim().max(50, 'Максимальная длина 50 символов'),
